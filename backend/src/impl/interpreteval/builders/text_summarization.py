...
class SummExplainaboardBuilder:
    ...
    def _generate_example(self) -> Iterable:
        ...
        with open(self.build_config.path_output_file, encoding="utf8") as fin:
            for id_, line in enumerate(fin):
                source_text, gold_text, hypo_text= line.split("\t")
                yield id_, {"source": source_text, "reference": gold_text, "hypothesis": hypo_text}
    ...
    def _complete_feature(self, feature_table_iterator):
        ...
        """
        inputs = []
        for _id, feature_table in feature_table_iterator:
