...
class TCExplainaboardBuilder:
    ...
    def __init__(self, info: SysOutputInfo = None,
                 ...
                 file_format = None,
                 system_output_object:List[dict] = None,
        ...
        self._system_output:List[dict] = system_output_object
        ...
        # print(self.build_config.path_output_file)
        if self.build_config.path_output_file:
            builder_loader = loader_registry[self._info.task_name][self._file_format]()
            self._system_output = builder_loader.load(path_system_output=self.build_config.path_output_file).system_output
        if not self.build_config.path_output_file and not system_output_object:
            raise ValueError("the path or the object of system output should be specified.")
    ...
    def _complete_feature(self):
        ...
        for _id, dict_sysout in enumerate(self._system_output):
            ...
            for bucket_feature in bucket_features:
                feature_value = eval(TCExplainaboardBuilder.get_bucket_feature_value(bucket_feature))(dict_sysout)
